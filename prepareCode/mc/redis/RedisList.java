...
public class RedisList<E> implements List<E> {
	...
	public RedisList(String name, RedisDAO redisDao) {
		this.nameKey = "list:" + name;
		this.namekeyBytes = this.nameKey.getBytes();
...
//	}
	...
	private List<E> getList() {
		...
		int size = size();
		if (size > 0) {
			List<byte[]> list = getRedisDAO().lrange(namekeyBytes, 0, size - 1);
	...
	public boolean add(E e) {
		...
		int size = size();
		Long l = getRedisDAO().rpush(namekeyBytes, eBytes);
		boolean b = false;
		if (l.intValue() > size) {
			b = true;
		}
	...
	public boolean remove(Object o) {
		...
		Long l = getRedisDAO().lrem(namekeyBytes, 1, oBytes);
		boolean b = false;
		if (l > 0) {
			b = true;
		}
	...
	public E get(int index) {
		byte[] eBytes = getRedisDAO().lindex(namekeyBytes, index);
	...
	public E set(int index, E element) {
		...
		getRedisDAO().lset(namekeyBytes, (long) index, eBytes);
		return old;
	}
	...
	public List<E> subList(int fromIndex, int toIndex) {
		List<byte[]> list = getRedisDAO().lrange(namekeyBytes, fromIndex, toIndex - 1);
		List<E> listRet = null;
