...
public class ClassPathResourceUtil {
	 ...
	 * <li>classpath:xx		返回的是单个ClassPathResource，URL查询顺序同classpath*:，具体由JDK或Tomcat(如ClassLoader.getResource())解析文件名得到</li>
	 * <li>绝对路径				返回的是单个UrlResource，URL=new URL(path)</li>
	 * 
	 * <p>
	 * 示例：
	 * 假如项目依赖：ec-incubator->ec-base->ec-utils，
	 * <p>情形1：每个项目都有myenv.xml.
	 * <li>classpath*:xx ec-incubator#myenv.xml</li>
	 * <li>classpath:xx ec-incubator#myenv.xml</li>
	 * <p>情形2：ec-incubator没有myenv.xml，ec-base和ec-utils都有myenv.xml.
	 * <li>classpath*:xx ec-base#myenv.xml</li>
	 * <li>classpath:xx ec-base#myenv.xml</li>
	 * 
	 * @param path 支持前缀"file://"、"classpath:"、"classpath*"（没有前缀等同于该前缀）；路径 支持Spring的通配符
	 *            示例：com/hzeool/☀☀/config/*.xml
	 * @return
	 */
	 ...
	 * 查询：若ec-slh2没有msg/msg.properties，结果数组中就没有ec-slh2
	 * Tomcat：
	 ...
	 * <li>classpath:msg/msg.properties：[ec-slh2]</li>
	 * 
	 * @param paths 支持前缀"file://"、"classpath:"、"classpath*"（没有前缀等同于该前缀）；路径 支持Spring的通配符
	 *            示例：com/hzeool/☀☀/config/*.xml
	 * @return
	 */
	...
	private static List<Resource> getResourceList(boolean includeAll, String... paths) {
		...
		for (String path : paths) {
			// 没有已知的前缀，则添加prefix前缀
			if (!path.startsWith(FilepathUtil.FILE_URL_PREFIX) 
					&& !path.startsWith(CLASSPATH_ALL_URL_PREFIX) 
					&& !path.startsWith(CLASSPATH_URL_PREFIX)) {
				path = prefix + path;
			}
			try {
