...
public class RequestContext implements Serializable {
	...
	public RequestContext(HttpServletRequest request) {
		...
		if (StringUtils.isNotBlank(queryString)) {
			...
			for (String k_v : k_vs) {
				String[] kv = k_v.split("=");
				this.queryParams.put(kv[0], kv.length > 1 ? kv[1] : "");
			}
		} else {
			this.queryParams = Collections.emptyMap();
		}
		...
		String hostPort = StringUtils.substringBefore(requestUrl, this.contextPath + "/");

		this.requestUrl = hostPort + contextPath;

		// hostName
		hostPort = StringUtils.substringAfter(hostPort, "://");
		String[] h_p = hostPort.split(":");
		if (h_p.length > 0) {
			this.serverHost = h_p[0];
		} else {
			this.serverHost = this.serverName;
		}
