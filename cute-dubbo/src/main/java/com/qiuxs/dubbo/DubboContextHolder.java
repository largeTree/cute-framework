...
public class DubboContextHolder {
	 ...
	 * @return dubboBean不为null时，返回dubboBean。否则通过beanName取dubbo bean。dubbo的服务提供者不存在时，将返回null。
	 */
    ...
    public static Map<String, List<Thread>> getThreads(State threadState) {
		...
		for (String port : portExecutorMap.keySet()) {
			...
			@SuppressWarnings("unchecked")
			Set<? extends AbstractQueuedSynchronizer> workers = (Set<? extends AbstractQueuedSynchronizer>) com.qiuxs.cuteframework.core.basic.utils.reflect.FieldUtils.getFieldValueQuietly(es, "workers");
			for (AbstractQueuedSynchronizer worker : workers) {
				Thread thread = (Thread) com.qiuxs.cuteframework.core.basic.utils.reflect.FieldUtils.getFieldValueQuietly(worker, "thread");
	...
	public static String getThreadPoolInfo(State state, boolean extFlag) {
		...
		StringBuilder sb = new StringBuilder(EnvironmentContext.getAppName()).append(" Thread Pool StackTrace: ").append(threadTraceMap.toJSONString());
		return sb.toString();
	}
}
