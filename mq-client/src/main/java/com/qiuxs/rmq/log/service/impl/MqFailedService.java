...
public class MqFailedService extends AbstractDataPropertyUKService<Long, MqFailed, MqFailedDao> implements IMqFailedService {
	...
	@Override
	protected void postCreate(MqFailed bean) {
		super.postCreate(bean);
		AsyncTaskExecutor.execute(new RunnableAsyncTask<MqFailed>(bean) {
			@Override
			public void execute(MqFailed preparParam) {
				NoticeLogger.error("消息消费失败, msgId = " + bean.getMsgId(), bean.getStacktrace());
			}
		}, true);
	}
